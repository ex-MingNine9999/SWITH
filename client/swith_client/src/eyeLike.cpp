...
eyeLocation findEyes(cv::Mat frame_gray, cv::Rect face) {
  ...
  int eye_region_height = face.width * (kEyePercentHeight/100.0); int eye_region_top = face.height * (kEyePercentTop/100.0);
...
//  findSkin(debugImage);
  ...
  //-- Show what you got
  if (faces.size() > 0) {
    ret = findEyes(frame_gray, faces[0]);
  }
